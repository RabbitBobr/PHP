    ...
    if (count($array_data) > 2)
    {
        ...
        for ($i = 0; $i < count($names); $i++)
        {
            ...
            <label>' . (($i+1)<10 ? ($i+1)."&nbsp;&nbsp;" : $i+1) . '</label>
                <input class="sale_product_name" name="sale_product_name[]" type="text" placeholder="Название товара" value="' . $names[$i] . '"/>
                <input class="sale_product_price" name="sale_product_price[]" type="text" placeholder="Стоимость" value="' . $prices[$i] . '"/>
                <input class="sale_product_size" name="sale_product_size[]" type="text" placeholder="Кол-во" value="' . $sizes[$i] . '" />
            </div>';
        };
        ...
        $result_string .= '<input type="submit" id="add_data_sale_submit" value="Отправить" />
